...
def main():
    ...
    if len(sys.argv) != 3:
        print("Usage: paste-clipboard-ultracopier.py copy/move <destination_directory>")
        sys.exit(1)
    ...
    destination = uri_to_path(urllib.parse.unquote(sys.argv[2]))
    ...
    if clipboard_data:
        ...
        command = f'/usr/bin/ultracopier {action} {sources_formatted} "{destination}"'
